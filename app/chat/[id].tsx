      ...
      const loadChat = async () => {
        ...
        try {
          if (isNewConversation && playerIdFromNew) {
            ...
            setMessages([]);
            console.log('Chat: [Load] new conversation for', playerIdFromNew);
            return;
          }
          ...
          if (otherUserId) {
            ...
            console.log('Chat: [Load] WARN could not parse otherUserId from', roomId);
          }

          console.log('Chat: [Load] querying messages room_id=', roomId);
          ...
          if (error) {
            ...
            console.log('Chat: [Load] fetched', data?.length ?? 0, 'rows');
      ...
      const buildChannel = () => {
        ...
        state.channel = supabase
          ...
          .subscribe((status, err) => {
            if (err) {
              console.log('Chat: [RT] ERROR', err.message ?? err);
            } else {
              console.log('Chat: [RT] status', status);
            }
            if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
              console.log('Chat: [RT] retry in 3s…');
          ...
          >
            {isImg && imgUri ? (
              <Image
                ...
                contentFit="cover"
                onError={(e) => console.log('Chat: image load error', e.error)}
              />
            ) : (
              <Text
                style={[
  ...
  function createStyles(colors: ThemeColors) {
    return StyleSheet.create({
      ...
      headerAvatar: { width: 34, height: 34, borderRadius: 17, backgroundColor: colors.surfaceLight },
      headerName: { fontSize: 16, fontWeight: '600' as const, color: colors.textPrimary },
      ...
      messageBubbleAvatar: { width: 28, height: 28, borderRadius: 14, backgroundColor: colors.surfaceLight },
      ...
      messageBubbleMe: { backgroundColor: colors.gold, borderBottomRightRadius: 4 },
      messageBubbleOther: { backgroundColor: colors.surface, borderBottomLeftRadius: 4 },
      messageText: { fontSize: 15, lineHeight: 21 },
      messageTextMe: { color: colors.white },
      messageTextOther: { color: colors.textPrimary },
      messageTime: { fontSize: 10, marginTop: 4 },
      messageTimeMe: { color: 'rgba(255,255,255,0.6)', textAlign: 'right' as const },
